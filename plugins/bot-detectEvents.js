...
await this.sendMessage(m.chat, { text: `🚩 ${usuario} *Ha cambiado el nombre del grupo 𝑯𝒚𝒑𝒆𝒓´V*`, mentions: [m.sender], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 

} else if (chat.detect2 && m.messageStubType == 22) {
await this.sendMessage(m.chat, { text: `🚩 ${usuario} *Ha cambiado la imágen del grupo 𝑯𝒚𝒑𝒆𝒓´V*`, mentions: [m.sender] }, { quoted: fliveLoc, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 
...
let txt2 = `🚩 *Un admin menos 𝑯𝒚𝒑𝒆𝒓´V*\n\n`
